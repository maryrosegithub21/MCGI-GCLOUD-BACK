...
const projectId = process.env.PROJECT_ID; // Or set directly
...
async function startServer() {
    try {
        const apiKey = await accessSecretVersion('GEMINI_API_KEY');
        const spreadsheetId = await accessSecretVersion('GOOGLE_SHEETS_ID');
        const credentials = await accessSecretVersion('GOOGLE_APPLICATION_CREDENTIALS');
        const range = await accessSecretVersion('GOOGLE_SHEETS_RANGE');

        // Parse the credentials JSON string into an object
        ...
        process.env.GOOGLE_APPLICATION_CREDENTIALS = JSON.stringify(credentialsObject);
        process.env.GOOGLE_SHEETS_RANGE = range;
...
const model = genAI.getGenerativeModel({
  ...
  const generationConfig = {
      temperature: 1,
        ...
        // ... (Gemini chatbot endpoint code using process.env.GEMINI_API_KEY)
        app.post('/api/gemini-chat', async (req, res) => { 
            let message = req.body.message;
  console.log("Message type before conversion:", typeof message);
  console.log("Message before conversion:", message);
  ...
  console.log("Message after conversion:", message);
    ...
    if (result) {
        ...
        // ... (Any other endpoints or middleware)
